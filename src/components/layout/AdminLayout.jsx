...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "w-60" : "w-16"} flex-shrink-0 bg-card border-r border-border flex flex-col transition-all duration-300 fixed inset-y-0 left-0 z-40`}
      >
        {/* Logo */}
      ...
      {/* Main content */}
      <main
        className={`flex-1 ${sidebarOpen ? "ml-60" : "ml-16"} transition-all duration-300 min-h-screen`}
      >
        <Outlet />
      </main>
    </div>
