  ...
  useEffect(() => {
    if (storeSettings) {
      setForm({
        ...
        delivery_close:
          msToTime(storeSettings.delivery_close_time_ms) || "22:00",
        ...
        delivery_min_order:
          storeSettings.delivery_min_order?.toString() || "10",
        delivery_fee: storeSettings.delivery_fee?.toString() || "3.99",
        delivery_free_threshold:
          storeSettings.delivery_free_threshold?.toString() || "25",
        estimated_delivery_mins:
          storeSettings.estimated_delivery_mins?.toString() || "35",
        estimated_pickup_mins:
          storeSettings.estimated_pickup_mins?.toString() || "15",
      });
    }
  }, [storeSettings]);
        ...
        <div className="bg-card rounded-2xl border border-border p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                onCheckedChange={(v) => set("delivery_enabled", v)}
              />
              <Label>{form.delivery_enabled ? "Enabled" : "Disabled"}</Label>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <Input
                type="time"
                value={form.delivery_open}
                onChange={(e) => set("delivery_open", e.target.value)}
                className="mt-1.5"
              />
              <p className="text-xs text-muted-foreground mt-1">
                = {timeToMs(form.delivery_open).toLocaleString()} ms
              </p>
            </div>
            <div>
              <Label className="flex items-center gap-1">
              ...
              <Input
                type="time"
                value={form.delivery_close}
                onChange={(e) => set("delivery_close", e.target.value)}
                className="mt-1.5"
              />
              <p className="text-xs text-muted-foreground mt-1">
                = {timeToMs(form.delivery_close).toLocaleString()} ms
              </p>
            </div>
          </div>
          <div className="p-3 bg-muted/50 rounded-xl mb-4 flex items-start gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              Times stored as milliseconds from midnight. Open:{" "}
              {timeToMs(form.delivery_open).toLocaleString()} ms · Close:{" "}
              {timeToMs(form.delivery_close).toLocaleString()} ms
            </p>
          </div>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => set("delivery_free_threshold", e.target.value)}
                className="mt-1.5"
              />
            </div>
          </div>
          <div className="mt-4">
            ...
            <Input
              ...
              onChange={(e) => set("estimated_delivery_mins", e.target.value)}
        ...
        <div className="bg-card rounded-2xl border border-border p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Label>{form.pickup_enabled ? "Enabled" : "Disabled"}</Label>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <Input
                type="time"
                value={form.pickup_open}
                onChange={(e) => set("pickup_open", e.target.value)}
                className="mt-1.5"
              />
              <p className="text-xs text-muted-foreground mt-1">
                = {timeToMs(form.pickup_open).toLocaleString()} ms
              </p>
            </div>
            <div>
              ...
              <Input
                type="time"
                value={form.pickup_close}
                onChange={(e) => set("pickup_close", e.target.value)}
                className="mt-1.5"
              />
              <p className="text-xs text-muted-foreground mt-1">
                = {timeToMs(form.pickup_close).toLocaleString()} ms
              </p>
            </div>
          </div>
          <div>
            <Label>Est. Pickup Time (mins)</Label>
            <Input
              type="number"
              value={form.estimated_pickup_mins}
              onChange={(e) => set("estimated_pickup_mins", e.target.value)}
              className="mt-1.5 w-40"
            />
          </div>
        </div>

        <Button className="w-full h-12 font-bold gap-2" onClick={handleSave}>
