...
import { Package, Clock, CheckCircle2, Truck, ChefHat, XCircle, Flame, FileText } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="space-y-4">
        {orders.map((order, index) => {
          ...
          return (
            <motion.div key={order.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="bg-card rounded-2xl border border-border p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                <div>
                  <p className="text-sm text-muted-foreground">{order.created_date && format(new Date(order.created_date), 'MMM d, yyyy · h:mm a')}</p>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 mb-4">
                {order.items?.map((item, i) => (
                  <div key={i} className="flex items-center gap-3 bg-muted/50 rounded-xl p-3 flex-shrink-0">
                    <div className="w-12 h-12 rounded-lg bg-muted overflow-hidden">
                      {item.image_url ? <img src={item.image_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Flame className="w-6 h-6 text-muted-foreground/30" /></div>}
                    </div>
                    <div>
                      <p className="text-sm font-medium">{item.product_name}</p>
                      <p className="text-xs text-muted-foreground">x{item.quantity} · ${((item.price || 0) * (item.quantity || 1)).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground capitalize">{order.order_type?.replace('_', ' ')} · {order.payment_method}</span>
